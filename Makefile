...
recycle_tables: ## Recreating the ClickHouse tables
	docker-compose -f ${dc_path} exec ${dev_clickhouse_server_container} clickhouse-client -q "${dropSql}"
	docker-compose -f ${dc_path} exec ${dev_clickhouse_server_container} clickhouse-client -q "${createSql}"
