...
recycle_tables: ## Recreating the ClickHouse tables
	docker-compose -f ${dc_path} exec -T ${dev_clickhouse_server_container} clickhouse-client --host ${dev_clickhouse_server_container} -q "${dropSql}"
	docker-compose -f ${dc_path} exec -T ${dev_clickhouse_server_container} clickhouse-client --host ${dev_clickhouse_server_container} -q "${createSql}"
